            ...
            try {
                for (;;) {
                    ...
                    if (msgParts[Constants.MSG_WORDSTATE_INDEX].equals("null")) {
                        // no game ongoing
                        outputHandler.handleMsg("No game ongoing. Type \"start\" to start a new game.");
                    } else if(msgParts[Constants.MSG_WORDSTATE_INDEX].indexOf("_") == -1) {
                        // word guessed right
                        outputHandler.handleMsg("You win! The word was \"" + msgParts[Constants.MSG_WORDSTATE_INDEX] + "\". Your score is now " + msgParts[Constants.MSG_SCORE_INDEX] + ". Type \"start\" to start a new round.");
                    } else {
                        // word not guessed right
                        if (msgParts[Constants.MSG_ATTEMPTS_INDEX].equals("0")) {
                            // all guesses used
                            outputHandler.handleMsg("You lost! Your score is now " + msgParts[Constants.MSG_SCORE_INDEX] + ". Type \"start\" to start a new round.");
                        } else {
                            // there are guesses left
                            outputHandler.handleMsg("Game ongoing. Wordstate:" + msgParts[Constants.MSG_WORDSTATE_INDEX] + ", Attempts left:" + msgParts[Constants.MSG_ATTEMPTS_INDEX] + ", Score:" + msgParts[Constants.MSG_SCORE_INDEX]);
                        }
                    }
                }
